...
```text
{ columns: TableProps['columns'] } => TableProps['columns']
```

Essentially, you pass the existing columns as a **named** prop and you get back
a set of enhanced columns that you can use within the Table

```jsx harmony
import React from 'react';
import Card from 'components/Card';
import useEnumerableTableRows from 'utils/useEnumerableTableRows';
...
const Example = () => {
  const enumerableColumns = useEnumerableTableRows({ columns });
