A thing to remember is that all the comboboxes have a controlled state when
it comes to the selected items. Thus, they do not maintain their own internal state
but expect you to pass them explicitly the selected items.

You get notified of changes to the selections through the `onChange` prop.

---

A MultiCombobox can be basic:

```typescript jsx
const Example3 = () => {
  const [selectedCar, updateSelectedCar] = React.useState([]);
  const [selectedMoto, updateSelectedMoto] = React.useState([]);
  return (
    <>
      <Box width={0.4} mr={4}>
        <MultiCombobox
          label="Choose a car manufacturer"
          items={['Toyota', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hammer', 'Dodge', 'Audi']}
          onChange={updateSelectedCar}
          value={selectedCar}
          placeholder="Select manufacturers"
        />
      </Box>
      <Box width={0.5}>
        <MultiCombobox
          label="Choose a car manufacturer"
          hideLabel
          placeholder="Choose a motorcycle manufacturer"
          items={['Benelli', 'Ducati', 'BMW', 'Moto Guzzi', 'Piaggio']}
          onChange={updateSelectedMoto}
          value={selectedMoto}
        />
      </Box>
    </>
  );
};

<Example3 />;
```

A MultiCombobox can have disabled options:

```typescript jsx
const Example3 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([]);
  return (
    <Box width={0.3}>
      <MultiCombobox
        label="Choose a car manufacturer"
        items={['Toyota', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hammer', 'Dodge', 'Audi']}
        onChange={updateSelectedItems}
        value={selectedItems}
        placeholder="Select manufacturers"
        disableItem={item => item === 'Toyota' || item === 'BMW'}
      />
    </Box>
  );
};

<Example3 />;
```

A MultiCombobox have search functionality:

```typescript jsx
const Example4 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([]);
  return (
    <Box width={0.3}>
      <MultiCombobox
        searchable
        label="Choose a car manufacturer"
        items={['Toyota', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hammer', 'Dodge', 'Audi']}
        onChange={updateSelectedItems}
        value={selectedItems}
        placeholder="Search for a manufacturer"
      />
    </Box>
  );
};

<Example4 />;
```

A MultiCombobox can allow user to add values of their own:

```typescript jsx
const Example4 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([]);
  return (
    <Box width={0.3}>
      <MultiCombobox
        allowAdditions
        searchable
        label="Choose a car manufacturer"
        items={[]}
        onChange={updateSelectedItems}
        value={selectedItems}
        placeholder="Search for a manufacturer"
      />
    </Box>
  );
};

<Example4 />;
```

A MultiCombobox can allow user to only add values that satisfy a certain condition:

```typescript jsx
const Example4 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([]);
  return (
    <Box width={0.5}>
      <MultiCombobox
        allowAdditions
        searchable
        label="Only words that contain the word Panther"
        items={[]}
        onChange={updateSelectedItems}
        validateAddition={text => text.toLowerCase().includes('panther')}
        value={selectedItems}
        placeholder="Must contain the word Panther"
      />
    </Box>
  );
};

<Example4 />;
```

A MultiCombobox can have a clear all option after a number of values are added (in example 4):

```typescript jsx
const Example5 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([
    'Toyota',
    'Ford',
    'Chevrolet',
    'BMW',
  ]);
  const [selectedMoto, updateSelectedMoto] = React.useState([
    'Benelli',
    'Ducati',
    'BMW',
    'Moto Guzzi',
  ]);
  return (
    <>
      <Box width={0.4} mr={4}>
        <MultiCombobox
          searchable
          label="Choose a car manufacturer"
          items={['Toyota', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hammer', 'Dodge', 'Audi']}
          onChange={updateSelectedItems}
          value={selectedItems}
          placeholder="Search for a manufacturer"
          canClearAllAfter={4}
        />
      </Box>
      <Box width={0.4}>
        <MultiCombobox
          label="Choose a car manufacturer"
          hideLabel
          variant="solid"
          placeholder="Choose a motorcycle manufacturer"
          items={['Benelli', 'Ducati', 'BMW', 'Moto Guzzi', 'Piaggio']}
          onChange={updateSelectedMoto}
          value={selectedMoto}
          canClearAllAfter={4}
        />
      </Box>
    </>
  );
};

<Example5 />;
```

A MultiCombobox can have "solid" variant:

```typescript jsx
const Example5 = () => {
  const [selectedItems, updateSelectedItems] = React.useState([]);
  return (
    <Box width={0.3}>
      <MultiCombobox
        searchable
        variant="solid"
        label="Choose a car manufacturer"
        items={['Toyota', 'Ford', 'Chevrolet', 'BMW', 'Mercedes', 'Hammer', 'Dodge', 'Audi']}
        onChange={updateSelectedItems}
        value={selectedItems}
        placeholder="Search for a manufacturer"
      />
    </Box>
  );
};

<Example5 />;
```

A MultiCombobox can have arbitrary nested items:

```typescript jsx
const Example6 = () => {
  const [selectedCar, updateSelectedCar] = React.useState([]);
  const [selectedMoto, updateSelectedMoto] = React.useState([]);
  return (
    <>
      <Box width={0.4} mr={4}>
        <MultiCombobox
          label="Choose a car"
          searchable
          items={[
            {
              label: 'Toyota',
              subItems: [
                { label: 'Compact', subItems: ['Yaris', 'Aygo'] },
                { label: 'Family', subItems: ['Auris', 'Corolla'] },
              ],
            },
            {
              label: 'Ford',
              subItems: [
                { label: 'Trucks', subItems: ['Ranger', 'Raptor'] },
                { label: 'Compact', subItems: ['Focus', 'Fiesta'] },
              ],
            },
            'Chevrolet',
            'BMW',
            'Mercedes',
            'Hammer',
            'Dodge',
            'Audi',
          ]}
          onChange={updateSelectedCar}
          value={selectedCar}
          placeholder="Select cars"
        />
      </Box>
      <Box width={0.5}>
        <MultiCombobox
          label="Choose a motorcycle"
          hideLabel
          placeholder="Choose a motorcycle"
          items={[
            {
              label: 'Ducatti',
              subItems: ['Monster', 'Panigale'],
            },
            {
              label: 'Honda',
              subItems: ['Cbr 1000', 'Vfr 1200'],
            },
            {
              label: 'Yamaha',
              subItems: ['Fazer', 'R1', 'R6'],
            },
            {
              label: 'Suzuki',
              subItems: ['Gsxr', 'Intruder'],
            },
            {
              label: 'Kawasaki',
              subItems: ['Ninja 1000', 'Ninja 400', 'Ninja 600', 'Vulkan', 'H2r'],
            },
          ]}
          onChange={updateSelectedMoto}
          value={selectedMoto}
        />
      </Box>
    </>
  );
};

<Example6 />;
```
